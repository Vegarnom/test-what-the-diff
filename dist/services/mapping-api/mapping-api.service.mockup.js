...
exports.mockupMappingApiService = exports.mappingApiStub = void 0;
const constant_1 = require("../../common/helper/constant");
const response_data_1 = require("../../common/params/response.data");
const create_request_param_dto_1 = require("../request-param/dto/create-request-param.dto");
const create_response_param_dto_1 = require("../response-param/dto/create-response-param.dto");
...
const requestParams = new create_request_param_dto_1.CreateRequestParamDto();
requestParams.fromRequestParam(requestParam);
requestParams.children.push(new create_request_param_dto_1.CreateRequestParamDto().fromRequestParam(requestParamChild1), new create_request_param_dto_1.CreateRequestParamDto().fromRequestParam(requestParamChild2));
const responseParam = {
    id: 10003,
    paramDefault: '',
    paramChange: 'checkin',
...
const responseParams = new create_response_param_dto_1.CreateResponseParamDto();
responseParams.fromRequestParam(responseParam);
responseParams.children.push(new create_response_param_dto_1.CreateResponseParamDto().fromRequestParam(responseParamChild1), new create_response_param_dto_1.CreateResponseParamDto().fromRequestParam(responseParamChild2));
const mappingApiStub = () => {
    return {
        id: 10000,
        endpoint: 'findjson',
        requestHeaderParam: '',
...
exports.mappingApiStub = mappingApiStub;
exports.mockupMappingApiService = {
...
const create = (createMappingApiDto) => {
    let resData = new response_data_1.ResponseData();
    if (createMappingApiDto.requestParams.length > 0 &&
        createMappingApiDto.responseParams.length > 0) {
        resData = checkRequestAndResponseParams(createMappingApiDto.requestParams, createMappingApiDto.responseParams);
        if (resData.hasError) {
            return resData;
        }
        const mappingApi = new mapping_api_entity_1.MappingApi();
...
const findAllByTypeBrandId = (id) => {
    const resData = new response_data_1.ResponseData();
    const mappingApi = new mapping_api_entity_1.MappingApi();
    mappingApi.fromCreateMappingApiDto((0, exports.mappingApiStub)());
    resData.appData = [mappingApi];
...
const update = (id, createMappingApiDto) => {
    let resData = new response_data_1.ResponseData();
    if (id < 1) {
        resData.hasError = true;
        resData.message = 'Mapping api id is not validate';
...
const request = (key, endpoint, method, type, body) => {
    const mappingApi = new mapping_api_entity_1.MappingApi();
    mappingApi.fromCreateMappingApiDto((0, exports.mappingApiStub)());
...
const mappingParamRequest = (body, requestParams) => {
    let request = {};
        ...
        if (requestParams[i].paramDefault && requestParams[i].paramChange) {
            if (requestParams[i].children.length > 0) {
                if (requestParams[i].type.length > 1) {
                    ...
                    request[requestParams[i].paramChange].push(mappingParamRequest(body[requestParams[i].paramDefault][0], requestParams[i].children));
                }
                else {
                    request[requestParams[i].paramChange] = mappingParamRequest(body[requestParams[i].paramDefault], requestParams[i].children);
                }
            }
            else {
            ...
            if (requestParams[i].children.length > 0) {
                if (requestParams[i].type.length > 1) {
                    request = Object.assign(Object.assign({}, request), mappingParamRequest(body[requestParams[i].paramDefault][0], requestParams[i].children));
                }
                else {
                    request = Object.assign(Object.assign({}, request), mappingParamRequest(body[requestParams[i].paramDefault], requestParams[i].children));
                }
            }
        }
            ...
            if (requestParams[i].children.length > 0) {
                if (requestParams[i].type.length > 1) {
                    request[requestParams[i].paramChange] = [];
                    request[requestParams[i].paramChange].push(mappingParamRequest(body, requestParams[i].children));
                }
                else {
                    request[requestParams[i].paramChange] = mappingParamRequest(body, requestParams[i].children);
                }
            }
        }
...
const mappingParamResponse = (body, responseParams) => {
    let response = {};
        ...
        if (responseParams[i].paramDefault && responseParams[i].paramChange) {
            if (responseParams[i].children.length > 0) {
                if (responseParams[i].type.length > 1) {
                    ...
                    response[responseParams[i].paramChange].push(mappingParamResponse(body[responseParams[i].paramDefault][0], responseParams[i].children));
                }
                else {
                    response[responseParams[i].paramChange] = mappingParamResponse(body[responseParams[i].paramDefault], responseParams[i].children);
                }
            }
            else {
            ...
            if (responseParams[i].children.length > 0) {
                if (responseParams[i].type.length > 1) {
                    response = Object.assign(Object.assign({}, response), mappingParamResponse(body[responseParams[i].paramDefault][0], responseParams[i].children));
                }
                else {
                    response = Object.assign(Object.assign({}, response), mappingParamResponse(body[responseParams[i].paramDefault], responseParams[i].children));
                }
            }
        }
            ...
            if (responseParams[i].children.length > 0) {
                if (responseParams[i].type.length > 1) {
                    response[responseParams[i].paramChange] = [];
                    response[responseParams[i].paramChange].push(mappingParamResponse(body, responseParams[i].children));
                }
                else {
                    response[responseParams[i].paramChange] = mappingParamResponse(body, responseParams[i].children);
                }
            }
        }
...
const checkRequestAndResponseParams = (requestParams, responseParams) => {
    const resData = new response_data_1.ResponseData();
...
const detect = (createRequestParamDtoList) => {
...
const detectType = (requestParam) => {
    const arr = requestParam.type.map((element) => {
        return element.toLowerCase();
    });
    if (!checkIncluded(constant_1.TYPE_DATA, arr)) {
        return false;
    }
    if ((!checkIncluded(arr, ['object']) && requestParam.children.length > 0) ||
...
const checkIncluded = (myArray, checkingArray) => {
...
const findElementById = (requestParams, id) => {
    for (let i = 0; i < requestParams.length; i++) {
...
//# sourceMappingURL=mapping-api.service.mockup.js.map